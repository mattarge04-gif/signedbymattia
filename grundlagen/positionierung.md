# Positionierung

Stand: 24.09.2026 · Status: **entschieden** · Zielplan KW 39
(„Positionierung in fünf Sätzen: welches Problem du löst, für wen, warum du“)

## Was schon feststeht [F]

- Agenturname: **signedbymattia**. Im Aufbau, noch ohne eigene Website,
  Portfolio und geschäftliche Kontaktadresse (Agency-Masterplan §1).
- **Websites öffnen die Tür**, Automationen kommen als Zusatz dazu, ohne den
  Einstieg zu überladen.
- Region: **Root LU, 15 km Radius**, Rontal und Umgebung.
- Stärke laut Projektnotiz: **Opportunity Discovery**, also einen konkreten
  Engpass erkennen, ihn gestalterisch und funktional lösen und die Qualität
  beurteilen.
- Keine Cold Calls, keine breite Spam-Akquise.
- Versprochen werden Gestaltung, Klarheit, Funktion und ein überprüfbarer
  Prozess, **keine** garantierten Rankings und keine erfundenen Mehrumsätze.

## Arbeitshypothese [A] (Projektnotiz 23.09.2026)

> Für kleine, eigenständige Betriebe und kreative Projekte in Luzern/Rontal und
> passender Umgebung macht signedbymattia gute Arbeit online sichtbar und den
> nächsten Kontakt einfach. Bei einem wirklich wiederkehrenden Engpass verbindet
> Mattia die Website mit einer kleinen, prüfbaren Automation.

**Mögliche Lücke gegenüber der Konkurrenz [A]:** ein nachvollziehbarer Weg vom
konkreten Anfrageproblem über sichtbar begründete Design- und
Ablaufentscheidungen bis zu Umfang, Preis und ehrlichem Resultat
(`konkurrenz.md`).

**Möglicher Vorteil [A]:** Mattias Hintergrund in der Gebäudetechnik
(Streamline) macht ihn für HLKS-Betriebe glaubwürdiger als eine allgemeine
Webagentur. Die Suche „Website Sanitär/Heizung“ zeigte am 24.09.2026 nur
deutsche Anbieter, keine Schweizer.

## Entschieden am 24.09.2026 [F]

**Richtung: kreative Projekte und kleine Betriebe.** signedbymattia tritt nach
aussen für eigenständige Betriebe, Selbstständige und kreative Projekte auf;
die **Gestaltung steht im Vordergrund**. Verworfen: reiner HLKS-Spezialist,
allgemeine lokale Webagentur, „lokal mit HLKS als Spitze“.

**Auftritt:** in der Ich-Form als Mattia (`marke/tonalitaet.md`).

**HLKS bleibt als Branche** (Entscheid 24.09.2026): Der Lead Bot sucht weiter
HLKS-Betriebe zuerst, die Anzeigengruppe HLKS bleibt. Nach aussen wird HLKS
nicht hervorgehoben; die Marke bleibt „kreative Projekte und kleine Betriebe“.

## Die fünf Sätze [F] (von Mattia bestätigt am 24.09.2026)

1. **Problem:** Viele kleine Betriebe und kreative Projekte machen gute Arbeit,
   aber ihre Website zeigt das nicht, und der Weg zur Anfrage ist umständlich.
2. **Für wen:** Für eigenständige Betriebe, Selbstständige und kreative
   Projekte in Luzern und im Rontal.
3. **Was danach anders ist:** Ihre Arbeit wirkt online so gut wie in echt, und
   Interessierte finden in wenigen Sekunden den nächsten Schritt.
4. **Warum ich:** Ich gestalte jede Seite von Grund auf, begründe jede
   Entscheidung und verbinde sie dort, wo es sich lohnt, mit einer kleinen
   Automation, die Ihnen Arbeit abnimmt.
5. **Was ich bewusst nicht mache:** Keine Baukasten-Massenware und keine
   Versprechen zu Rankings oder Umsatz, die ich nicht belegen kann.

## Prüfen, bevor es gilt

- Versteht ein fremder Mensch die fünf Sätze in 30 Sekunden?
- Passt jeder Satz zu `zielkunden.md` und `angebote-preise.md`?
- Lässt sich jeder Satz mit einem Beleg zeigen, sobald es Beispiele gibt?
